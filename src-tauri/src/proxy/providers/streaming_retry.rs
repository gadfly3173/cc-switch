...
/// 带客户端转换选项的 Responses SSE 有界重连包装器。
///
/// 重试必须复用首轮的 Web Search 与隐藏 thinking 转换选项，否则重连后
/// 的同一响应会改变协议内容，客户端会看到前后不一致的消息。
pub(crate) fn create_resilient_anthropic_sse_stream_from_responses_with_web_search_options_for_client(
    initial: ByteStream,
    reconnector: Option<StreamReconnector>,
    ...
    async_stream::stream! {
        let Some(reconnector) = reconnector else {
            ...
            let translated = create_anthropic_sse_stream_from_responses_with_web_search_options_for_client(
                initial,
